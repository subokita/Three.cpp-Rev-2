...
static int glfons__renderCreate(void* userPtr, int width, int height)
{
    ...
    std::cout << gl->tex << std::endl;
    std::cout << gl->bufferIDs[0] << std::endl;
    std::cout << gl->bufferIDs[1] << std::endl;
...
static void glfons__renderDraw(void* userPtr, const float* verts, const float* tcoords, const unsigned int* colors, int nverts)
{
    ...
    glBindBuffer( GL_ARRAY_BUFFER, gl->bufferIDs[0] );
    glBufferData( GL_ARRAY_BUFFER, sizeof( float ) * 2 * nverts, verts, GL_STATIC_DRAW );
    glEnableVertexAttribArray( 0 );
    glVertexAttribPointer( 0, 2, GL_FLOAT, GL_FALSE, 0, (void*) 0 );
    
    glBindBuffer( GL_ARRAY_BUFFER, gl->bufferIDs[1] );
...
static void glfons__renderDelete(void* userPtr)
{
    ...
    if( gl->bufferIDs[0] != 0 ) {
        glDeleteBuffers(2, gl->bufferIDs );
    }
